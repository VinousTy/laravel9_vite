        ...
        <div class="max-w-2xl mx-auto py-4">
          ...
          <form method="POST" action="{{ route('events.update', ['event' => $event->id]) }}">
            ...
            <div class="mt-4">
              ...
              <x-textarea row="3" id="information" class="block mt-1 w-full" name="information">{{ $event->information
                }}
              </x-textarea>
            </div>
            ...
            <div class="flex items-center justify-end mt-4">
              <div class="mt-4">
                <x-jet-label for="max_people" value="{{ __('店員数') }}" />
                <x-jet-input id="max_people" class="block mt-1 w-full" type="number" name="max_people"
                  value="{{ $event->max_people }}" required />
              </div>
              <div class="flex space-x-4 justify-around">
                <input type="radio" name="is_visible" value="1" @if ($event->is_visible === 1)
                checked
                @endif
                >表示
                <input type="radio" name="is_visible" value="0" @if ($event->is_visible === 0)
                checked
                @endif>非表示
              </div>
